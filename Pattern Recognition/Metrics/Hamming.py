...
for j in range(0, len(csv_test.index)):
    ...
    for i in range(0, len(csv_train.index)):
        ...
        #Находим расстояние между объектом обучающей и тестовой выборок по метрике Хэмминга
        #(1 - сумма количества одинаковых значений соответствующих свойств, деленная на общее количество свойств)
        cur_min = 1 - sum(1 for a, b in zip(csv_test_row, csv_train_row) if a == b)/9
    ...
    print(csv_test.iloc[j, 0] + " is a " + overall_min_category + ", Hamming distance: " + str(overall_min) + ", Nearest Neighbour: " + nearest_neighbour)
input("Press any key...")