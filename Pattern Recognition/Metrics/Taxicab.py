...
for j in range(0, len(csv_test.index)):
    ...
    for i in range(0, len(csv_train.index)):
        ...
        #Находим расстояние между объектом обучающей и тестовой выборок по метрике городских кварталов
        #(сумма модулей разности значений соответствующих свойств)
        cur_min = sum([abs(a - b) for a, b in zip(csv_test_row, csv_train_row)])
    ...
    print(csv_test.iloc[j, 0] + " is a " + overall_min_category + ", Taxicab distance: " + str(overall_min) + ", Nearest Neighbour: " + nearest_neighbour)
input("Press any key...")