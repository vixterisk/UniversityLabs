...
for j in range(0, len(csv_test.index)):
    ...
    for i in range(0, len(csv_train.index)):
        ...
        #Находим расстояние между объектом обучающей и тестовой выборок по метрике Джаккарда
        #(1 - частное пересечения множеств свойств и их объединения)
        intersection = len(list(set(csv_test_row).intersection(csv_train_row)))
        union = (len(csv_test_row) + len(csv_train_row)) - intersection
        cur_min = 1 - (float(intersection) / union)
    ...
    print(csv_test.iloc[j, 0] + " is a " + overall_min_category + ", jaccard distance: " + str(overall_min) + ", Nearest Neighbour: " + nearest_neighbour)
input("Press any key...")